...
const int CHUNK_X = WORLD_SIZE_X / CHUNK_SIZE; // 64
const int CHUNK_Y = WORLD_SIZE_Y / CHUNK_SIZE; // 8
const int CHUNK_Z = WORLD_SIZE_Z / CHUNK_SIZE; // 64

// 视距（chunk 半径，越大越吃性能）
const int VIEW_CHUNK_RADIUS = 6; // 6 => 13x13 chunks 可见（可改）
...
// 物理
const float PLAYER_RADIUS = 0.3f;    // 半宽 ~0.6格
const float PLAYER_HEIGHT = 1.8f;    // MC 类似
const float GRAVITY = 20.0f;
const float WALK_SPEED = 4.3f;       // 方块/秒（近似）
const float JUMP_HEIGHT = 1.2f;      // 你要求 1.2格
...
// ===========================
// Chunk 世界：存 block type（uint8）
// + 只绘制可见面（邻居是AIR才画）
// + Chunk 缓存 display list（或立即模式）
// ===========================
...
void generateChunk(int cx,int cy,int cz){
    ...
    for(int lx=0; lx<CHUNK_SIZE; ++lx){
        for(int lz=0; lz<CHUNK_SIZE; ++lz){
            ...
            // 树（只在地表 chunk 里处理：避免跨 chunk 太复杂，这里简化）
            if(cy == (h/CHUNK_SIZE)){
                if(!isSandBand){
...
void emitFace(int x,int y,int z, int face, uint8_t t){
    ...
    auto applyVertexLight = [&](float ao){
        ...
        glColor4f(glGetFloatv?0:1,0,0,1); // 防止某些编译器警告（不影响）
        // 重新设置颜色（保持你原色基础上乘亮度）
        // 由于上面 setBlockColor 已经设置了当前颜色，这里用 glColor* 直接乘不方便，
        // 所以我们用 glGet 不可靠。改为：再次按类型设色后用 glColor4f 覆盖乘亮度。
        // ——直接写死各块颜色乘亮度：
        float r=1,g=1,b=1,a=1;
        switch(bt){
            case GRASS:   r=0.2f; g=0.8f; b=0.2f; break;
            case DIRT:    r=0.5f; g=0.3f; b=0.0f; break;
            case STONE:   r=0.6f; g=0.6f; b=0.6f; break;
            case WOOD:    r=0.6f; g=0.3f; b=0.0f; break;
            case LEAVES:  r=0.0f; g=0.6f; b=0.0f; break;
            case SAND:    r=0.9f; g=0.9f; b=0.6f; break;
            case WATER:   r=0.0f; g=0.4f; b=0.8f; a=0.5f; break;
            case COAL:    r=0.1f; g=0.1f; b=0.1f; break;
            case IRON:    r=0.7f; g=0.7f; b=0.7f; break;
            case GOLD:    r=1.0f; g=0.8f; b=0.0f; break;
            case DIAMOND: r=0.0f; g=0.8f; b=0.8f; break;
            default:      r=g=b=1.0f; break;
        }
        glColor4f(r*l, g*l, b*l, a);
    };
    ...
    if(face==0){ // +Z
        ...
        applyVertexLight(ao0); glVertex3f(X,   Y,   Z+1);
        applyVertexLight(ao1); glVertex3f(X+1, Y,   Z+1);
        applyVertexLight(ao2); glVertex3f(X+1, Y+1, Z+1);
        applyVertexLight(ao3); glVertex3f(X,   Y+1, Z+1);
    }else if(face==1){ // -Z
        glNormal3f(0,0,-1);
        float ao0 = aoCorner(occ( 1,0,-1), occ(0,-1,-1), occ( 1,-1,-1));
        float ao1 = aoCorner(occ(-1,0,-1), occ(0,-1,-1), occ(-1,-1,-1));
        float ao2 = aoCorner(occ(-1,0,-1), occ(0, 1,-1), occ(-1, 1,-1));
        float ao3 = aoCorner(occ( 1,0,-1), occ(0, 1,-1), occ( 1, 1,-1));
        applyVertexLight(ao0); glVertex3f(X+1, Y,   Z);
        applyVertexLight(ao1); glVertex3f(X,   Y,   Z);
        applyVertexLight(ao2); glVertex3f(X,   Y+1, Z);
        applyVertexLight(ao3); glVertex3f(X+1, Y+1, Z);
    }else if(face==2){ // +Y
        glNormal3f(0,1,0);
        ...
        applyVertexLight(ao0); glVertex3f(X,   Y+1, Z+1);
        applyVertexLight(ao1); glVertex3f(X+1, Y+1, Z+1);
        applyVertexLight(ao2); glVertex3f(X+1, Y+1, Z);
        applyVertexLight(ao3); glVertex3f(X,   Y+1, Z);
    }else if(face==3){ // -Y
        glNormal3f(0,-1,0);
        float ao0 = aoCorner(occ(-1,-1,0), occ(0,-1, 1), occ(-1,-1, 1));
        float ao1 = aoCorner(occ( 1,-1,0), occ(0,-1, 1), occ( 1,-1, 1));
        float ao2 = aoCorner(occ( 1,-1,0), occ(0,-1,-1), occ( 1,-1,-1));
        float ao3 = aoCorner(occ(-1,-1,0), occ(0,-1,-1), occ(-1,-1,-1));
        applyVertexLight(ao0); glVertex3f(X,   Y, Z);
        applyVertexLight(ao1); glVertex3f(X+1, Y, Z);
        applyVertexLight(ao2); glVertex3f(X+1, Y, Z+1);
        applyVertexLight(ao3); glVertex3f(X,   Y, Z+1);
    }else if(face==4){ // +X
        glNormal3f(1,0,0);
        ...
        applyVertexLight(ao0); glVertex3f(X+1, Y,   Z+1);
        applyVertexLight(ao1); glVertex3f(X+1, Y,   Z);
        applyVertexLight(ao2); glVertex3f(X+1, Y+1, Z);
        applyVertexLight(ao3); glVertex3f(X+1, Y+1, Z+1);
    }else if(face==5){ // -X
        glNormal3f(-1,0,0);
        float ao0 = aoCorner(occ(-1,0,-1), occ(-1,-1,0), occ(-1,-1,-1));
        float ao1 = aoCorner(occ(-1,0, 1), occ(-1,-1,0), occ(-1,-1, 1));
        ...
        float ao3 = aoCorner(occ(-1,0,-1), occ(-1, 1,0), occ(-1, 1,-1));
        applyVertexLight(ao0); glVertex3f(X, Y,   Z);
        applyVertexLight(ao1); glVertex3f(X, Y,   Z+1);
        applyVertexLight(ao2); glVertex3f(X, Y+1, Z+1);
        applyVertexLight(ao3); glVertex3f(X, Y+1, Z);
    }
...
void drawInventoryUI(){
    ...
    // 主背包 9x3（对应 items[0..26]）+ 额外一行 9 格（items[27..35]）=> 4行共36
    if(playerInventory.showInventory){
        ...
        for(int r=0;r<4;r++){
            for(int c=0;c<9;c++){
                ...
                float y = invY + (3-r)*(slot+gap); // 从上往下
...
// ===========================
// 合成配方（保留你的配方；你原“木镐”和“木斧”配方一样，这里不删）
// ===========================
...
bool aabbCollides(float px,float py,float pz){
    // 玩家AABB：x/z 半径 PLAYER_RADIUS，高 PLAYER_HEIGHT
...
struct RayHit{
    bool hit=false;
    int bx=0,by=0,bz=0;
    int px=0,py=0,pz=0; // 放置位置（上一个空气格）
};

RayHit raycast(float maxDist){
    RayHit rh;
    float ox=playerX, oy=playerY+1.62f, oz=playerZ; // 视线高度
...
void tryMoveAxis(float& px,float& py,float& pz, float dx,float dy,float dz){
    ...
    if(dy!=0){
        ...
        if(!aabbCollides(px,ny,pz)){
            ...
            if(step < 0) onGround = true; // 向下撞到 => 落地
            velY = 0;
        }
    }
}
...
void passiveMouse(int x,int y){
    ...
    playerRotX -= dy * MOUSE_SENS; // 向上移动 -> pitch增加（这里用减更符合直觉）
